<script>
import { h } from 'vue'
import { loadLocaleMessagesAsync } from "@/i18n"
export default {
    name: 'lang-selector',
    props: ['type'],
    data: function () {
        return {}
    },
    computed: {
        locale: function () {
            return this.$i18n.locale;
        },
        availableLangs: function () {
            return this.$i18n._availableLocales;
        },
        lang: function () {
            return this.availableLangs.filter(locale => {
                return locale.code === this.locale;
            })[0] || {}
        }
    },
    methods: {
        setLoading: function (val,text) {
            this.$store.commit('loading',val,text);
        },
        loadLocaleMessages: function (locale) {
            this.setLoading(true, 'GETTING LANGUAGE PACK...');
            //this.$snackbar.show({type: 'info', title: 'Loading ' + locale.name + ' language pack', description: 'Translations by ' + locale.authors.join('')});
            loadLocaleMessagesAsync(locale.code).then(() => {
                //localStorage.setItem('USER_PREFERED_LOCALE', locale);
                if (this.$route.name && this.$route.name) document.title = `${this.$t(`strings.${this.$route.name}`)} | ${this.$t('strings.app_cecilia_tools')}`;
                else document.title =  `${this.$t(`strings.app_main_menu`)} | ${this.$t('strings.app_cecilia_tools')}`;
                this.setLoading(false, 'CONNECTING...');
                document.activeElement.blur();
            })
        }
    },
    render: function () {
        return h('div', {class: 'select-wrapper noselect', onClick: (e) => e.target.focus(), tabindex: 0}, [
            h('div', {class: 'selected-lang'}, [
                h('span', {class: 'flag-box flag-'+this.lang.code, style: [{'margin-right': this.type===1?0:'5px'}] }),
                this.type!==1? h('span', {style: 'vertical-align: middle;'}, this.lang.name) : null,

                h('div', {class: 'select-options glass-container'}, this.availableLangs.filter(lang => lang.code !== this.locale).map( locale => {
                    return h('div', {class: 'option', onClick: () => this.loadLocaleMessages(locale) }, [ // do not set tabindex on this element, it will break the local change event
                        h('div', {class: 'flag-box flag-'+locale.code}),
                        h('div', {value: locale.code, selected: this.lang===locale.code}, locale.name)
                    ])
                }))
            ])
        ])
    }
}
</script>
<style scoped>
/******************** FLAGS FROM https://greasyfork.org/en/scripts/372675-flags-css/code ********************/
.flag-it {background-image: url(data:image/png;base64,);}
.flag-ja {background-image: url(data:image/png;base64,);}.flag-en {background-image: url(data:image/png;base64,);}
.flag-es {background-image: url(data:image/png;base64,);}
.flag-th {background-image: url(data:image/png;base64,);}
.flag-kr,
.flag-ko {background-image: url(data:image/png;base64,);}
.flag-pt {background-image: url(data:image/png;base64,);}
.flag-fr {background-image: url(data:image/png;base64,);}

.flag-de {background-image: url(data:image/png;base64,);}
.flag-zh-TW,
.flag-tw {background-image: url(data:image/png;base64,);}
.flag-zh,
.flag-cn {background-image: url(data:image/png;base64,);}
.select-wrapper {
    position: relative;
    display: inline-block;
    font-size: 20px;
    height: 35px;
    text-align: center;
    margin: 5px;
    vertical-align: text-bottom;
    border: solid;
    border-radius: 10px;
    padding: 0 18px;
}
.flag-box {
    display: inline-block;
    height: 100%;
    width: 20px;
    background-position: center;
    background-repeat: no-repeat;
    vertical-align: middle;
    background-size: contain;
    margin-right: 5px;
}
.selected-lang {
    display: inline-block;
    height: 100%;
    cursor: pointer;
}
.select-options {
    z-index: 1;
    position: absolute;
    display: flex;
    flex-direction: column;
    top: 100%;
    left: 0;
    left: 50%;
    transform: translateX(-50%);
    padding: 5px 0px;
    border-radius: 7px;
    visibility: hidden;
    opacity: 0;
    white-space: nowrap;
    cursor: pointer;
}
.select-options > .option > div {
    display: inline-block;
}
.select-wrapper:focus .select-options {
    visibility: visible;
    opacity: 1;
}
.select-options > .option {
    padding: 0 10px;
    height: 40px;
}
.selected-lang:hover .select-options > .option:hover {
    background-color: #000000aa;
}

@media only screen and (max-width: 601px) {
    .select-options {
        position: fixed;
        top: 50%;
        left: 50%;
        transform: translate(-50%, -50%) scale(1.5);
    }
}
</style>