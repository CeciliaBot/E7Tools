<template>
        ...
        <Teleport :to="teleport[0]">
            <div v-show="!isMinimized" ref="floating-window" v-bind="$attrs" :class="['floating-window', {'glass-container-2': glassWindow, 'flat-window': !glassWindow, 'full-screen': isFullScreen}]" @focusin="onFocus" @blur="onBlur" tabindex="0">
...
export default {
    ...
    mounted() {
        ...
        let width = this.size && this.size.width ? this.size.width : Math.min( window.innerWidth - 30, Math.max(window.innerWidth*0.5, 900) ) + 'px'
        let height = this.size && this.size.height ? this.size.height : Math.min( window.innerHeight, Math.max(window.innerHeight*0.5, 500) ) + 'px'
        var flatWidthValue = width;
        var flatHeightValue = height;
        if (typeof flatWidthValue === 'string' || flatWidthValue instanceof String) {
            flatWidthValue = flatWidthValue.slice(-1) !== '%' ? parseFloat(flatWidthValue) : parseFloat(flatWidthValue)*window.innerWidth
        }
        if (typeof flatHeightValue === 'string' || flatHeightValue instanceof String) {
            flatHeightValue = flatHeightValue.slice(-1) !== '%' ? parseFloat(flatHeightValue) : parseFloat(flatHeightValue)*window.innerHeight
        }

        let left = (window.innerWidth - flatWidthValue) / window.innerWidth * 100 / 2 + (1*windows.length) + '%'
        let top = (window.innerHeight - flatHeightValue) / window.innerHeight * 100 / 2 + (1*windows.length) + '%'
        ...
        if (p) {
            if (p.bottom) {
                floatingWindow.style.bottom = p.bottom
            } else {
                floatingWindow.style.top = p.top || top
            }
            if (p.right) {
                floatingWindow.style.right = p.right
            } else {
                floatingWindow.style.left = p.left || left
            }
        } else {
            floatingWindow.style.top = top
            floatingWindow.style.left = left
        }
        ...
        resize(e, rect, axes, ajust) {
            ...
            if (!ajust) {
                ...
                if (this['min'+size]>=new_size)
                    return;
                this.$refs['floating-window'].style[size] = new_size + 'px'
                this.$refs['floating-window'].style[pos] = Math.max(0, rect[pos] + (e[cord] - ajust[cord])) + 'px'
            }
        },
