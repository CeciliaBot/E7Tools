...
function checkHeroClass(team, HeroDB, match) {
    var o = {};
    for (var i = 0; i < team.length; i++) {
        var thisHeroClass = HeroDB[team[i]].role;
        if (!o[thisHeroClass]) o[thisHeroClass] = 0;
        o[thisHeroClass]++
    }
    for (const role in match) {
        if (!o[role] || o[role] < match[role])
            return false;
    }
...
function checkHeroAttribute(team, HeroDB, match) {
    var o = {};
    for (var i = 0; i < team.length; i++) {
        var attr = HeroDB[team[i]].attribute;
        if (!o[attr]) o[attr] = 0;
        o[attr]+=1;
    }
    for (const attr in match) {
        if (!o[attr] || o[attr] < match[attr])
            return false;
    }
...
onmessage = function(e) {
         ...
         var HeroDB = e.HeroDB,
            campList = e.campList,
            nRoles = countRoles(e.classe),
            nAttributes = countRoles(e.elemento);
         var isCartesian = e.cartesianLock.flat().length>0 ? true : false;
         console.log("Is cartesian product?",isCartesian,reqId)
            ...
            const nuovoCampSimulatorTeam2 = function(inputTeam, locked = [], opts = {}) {
                ...
                if (checkScDupe(e.locked)) return postMessage({id: reqId, error: "locked_sc_normal_err"});

                if (isCartesian === false) {
                    var hasAdvSettings = false;
                    if ( ((e.locked.length + nRoles) > 4 && !e.preferenzeRisultati.lockedMatter) || ((e.locked.length + nAttributes) > 4 && !e.preferenzeRisultati.lockedMatter) || nAttributes > 4) { // team size error
                        return postMessage({id: reqId, error: "team_size_exceeded"});
                    } else { // can calculate
                        var useExperimental = true; // set to false if the new loop is not working correctly
                        if (nRoles > 0 || nAttributes > 0 || e.debuffs.length > 0 || e.buffs.length > 0 || e.AoE === true || e.noS1debuffs === true || e.noDebuffs === true || e.mustIncludeDispel || e.mustIncludeCleanser || e.preferenzeRisultati.n > 2000) hasAdvSettings = true;
                        ...
                        if (useExperimental && hasAdvSettings === false && e.locked.length <= 1) { // use only with no advanced settings
                            if (Object.keys(topics_results).length === 0) { // create topics combos (once for page visit)
                                ...
                                for (var hero_id in HeroDB) {
                                    //var topic_combos = Combinatorics.combination(HeroDB[hero_id].camping.values,2).toArray();
                            ...
                            e.risultati = new Array(e.preferenzeRisultati.n).fill({morale: -100, opzioneMigliore1: "", opzioneMigliore2: "", migliorPG1: "", migliorPG2: "", team: []});
                            ...
                            if (e.type=="friendship"){ // calculate highest friendship -> Only 1 character supported, no advanced settings
                              for (var key in topics_results) {
                                      ...
                                      for (var w = 0; w < currTopicCombo[key.split("_")[1]].length; w++) {
                                          ...
                                          for (var y = 0; y < e.risultati.length; y++) {
                                              if (punteggio >= e.risultati[y].morale) {
                                                  ...
                                                  if (inTop === -1) {
                                                      e.risultati.splice(y, 0,  {morale: punteggio, opzioneMigliore1: topics[key.split("_")[0]], opzioneMigliore2: topics[key.split("_")[1]], migliorPG1: c1, migliorPG2: c2, team: team} );
                                ...
                                return postMessage({id: reqId, risultati: e.risultati}); //worker is done
                                                    ...
                                                    for (var i3 = i2+1; i3 < currTopicCombo.eroi.length; i3++) {
                                                        ...
                                                        for (var y = 0; y < e.risultati.length; y++) {
                                                            if (punteggio >= e.risultati[y].morale) {
                                                                ...
                                                                if (inTop === -1) {
                                                                    e.risultati.splice(y, 0,  {morale: punteggio, opzioneMigliore1: topics[key.split("_")[0]], opzioneMigliore2: topics[key.split("_")[1]], migliorPG1: c1, migliorPG2: c1, team: team} );
                                                                    e.risultati.splice(e.preferenzeRisultati.n, 1);
                                                                } else if (inTop >= y) {
                                                                    e.risultati.splice(inTop, 1);
                                                                    e.risultati.splice(y, 0,  {morale: punteggio, opzioneMigliore1: topics[key.split("_")[0]], opzioneMigliore2: topics[key.split("_")[1]], migliorPG1: c1, migliorPG2: c1, team: team} );
                            ...
                            e.risultati = new Array(e.preferenzeRisultati.n).fill({morale: -100, team: []});
                            var currIndex = 0;
                            var lastProgress = -1;
                            ...
                            Combinatorics.bigCombination(campList,4-e.locked.length).forEach(teamComb => {
                                ...
                                //    postMessage({id: reqId, "status": Math.round(currIndex * 100 / tot) });
                                ...
                                if (nRoles > 0 && !checkHeroClass(elementoFiltro, HeroDB, e.classe))
                                    return;

                                if (!e.locked.every(i => team.includes(i)))
                                    return;

                                if (nAttributes > 0 && !checkHeroAttribute(elementoFiltro, HeroDB, e.elemento))
                                    return;
                    ...
                    if ((e.cartesianLock.length + e.locked.length) < 4 && campList.length < 4-(e.cartesianLock.length + e.locked.length)) { // can't calculate not enough heroes to fill remaining slots
                        return postMessage({id: reqId, error: "not_enough_heroes"});
                    } else if ( ((e.cartesianLock.length + e.locked.length + nRoles) > 4 && !e.preferenzeRisultati.lockedMatter) || ((e.cartesianLock.length + e.locked.length + nAttributes) > 4 && !e.preferenzeRisultati.lockedMatter)) { // Too many locked heroes
                        return postMessage({id: reqId, error: "team_size_exceeded"});
                    } else { // can calculate 
                        e.risultati = new Array(e.preferenzeRisultati.n).fill({morale: -100, team: []});
                ...
                postMessage({id: reqId, risultati: e.risultati});
}